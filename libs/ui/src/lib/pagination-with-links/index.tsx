		...
		if (totalPageCount <= maxVisiblePages) {
			for (let i = 0; i < totalPageCount; i++) {
				...
				items.push(
					<PaginationItem key={i}>
						<PaginationLink onClick={() => handlePagination(i)} isActive={page === i}>
			...
			items.push(
				<PaginationItem key={0}>
					<PaginationLink onClick={() => handlePagination(0)} isActive={page === 0}>
	...
	return (
		<Pagination className={cn(className)}>
			<PaginationContent className="max-sm:gap-0">
				...
				<PaginationItem>
					<PaginationNext
						onClick={() => handlePagination(Math.min(page + 1, totalPageCount - 1))}
